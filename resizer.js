  ...
  add(target, options) {
    ...
    if (target) {
        ...
        if (target) {
          ...
          if (isNaN(angle)) {
            ...
            if (tmatrix && tmatrix !== 'none') {
              ...
              const matrix_a = matrixValues[0];
              const matrix_b = matrixValues[1];
      ...
      if (!document.querySelector('.resizer-container')) {
        const container = target.options.container || document.body;
        container.appendChild(target.resizer);
      }
      let handleMousedown = (e) => {
        ...
        const container = target.options.container || document.body;
        target.startPos = { x: e.clientX + container.scrollLeft, y: e.clientY + container.scrollTop };
        target.size = {
        ...
        if (e.type === 'touchstart') {
          ...
          const container = target.options.container || document.body;
          target.startPos = {
            x: e.touches[0].clientX + container.scrollLeft,
            y: e.touches[0].clientY + container.scrollTop,
          };
        }
      ...
      let handleMousemove = (e) => {
        ...
        const container = target.options.container || document.body;
        let x = e.clientX + container.scrollLeft - target.startPos.x;
        let y = e.clientY + container.scrollTop - target.startPos.y;
        if (e.type === 'touchmove') {
          x = e.touches[0].clientX + container.scrollLeft - target.startPos.x;
          y = e.touches[0].clientY + container.scrollTop - target.startPos.y;
        }
        ...
        if (options.boundWithContainer) {
          const container =
            options.boundWithContainer instanceof HTMLElement ? options.boundWithContainer : target.offsetParent;
          if (container) {
            const bBox = container.getBoundingClientRect();
            const xBox = target.getBoundingClientRect();

            if (xBox.left < bBox.left) {
              const dx = bBox.left - xBox.left;
              newX += dx / target.scale;
            }
            if (xBox.top < bBox.top) {
              const dy = bBox.top - xBox.top;
              newY += dy / target.scale;
            }
            if (xBox.right > bBox.right) {
              const dx = bBox.right - xBox.right;
              newX += dx / target.scale;
            }
            if (xBox.bottom > bBox.bottom) {
              const dy = bBox.bottom - xBox.bottom;
              newY += dy / target.scale;
            }
            target.style.left = newX + 'px';
            target.style.top = newY + 'px';
      ...
      target.handleMouseEnter = (e) => {
        if (this.target != e.target && e.target.options.isHoverLine) {
          ...
          hoverLine.style.left = parseInt(bBox.left - bBoxContainer.left) + 'px';
          hoverLine.style.top = parseInt(bBox.top - bBoxContainer.top) + 'px';
  ...
  show(target) {
    if (target && target.resizer) {
      if (!this.resizer) {
        ...
        container.appendChild(ddmrr_angle_div);
        ...
        let handleMousemove = (e) => {
          ...
          let x = e.clientX + container.scrollLeft - resizer.startPos.x;
          let y = e.clientY + container.scrollTop - resizer.startPos.y;
          ...
          if (e.type === 'touchmove') {
            const container = target.options.container || document.body;
            x = e.touches[0].clientX + container.scrollLeft - resizer.startPos.x;
            y = e.touches[0].clientY + container.scrollTop - resizer.startPos.y;
            X = e.touches[0].clientX + container.scrollLeft;
            Y = e.touches[0].clientY + container.scrollTop;
          }
          if (angle) {
          ...
          if (target.options.boundWithContainer) {
            const container =
              target.options.boundWithContainer instanceof HTMLElement
                ? target.options.boundWithContainer
                : target.offsetParent;
            if (container) {
              const bBox = container.getBoundingClientRect();
              const xBox = target.getBoundingClientRect();
              if (
                xBox.left < bBox.left ||
                xBox.top < bBox.top ||
                xBox.right > bBox.right ||
                xBox.bottom > bBox.bottom
              ) {
      ...
      let updateResizerCursor = (angle) => {
        if (angle != this.resizer.angle) {
          ...
          resizer.ne_resizer.className =
            resizer.ne_resizer.className.replace(/ cursor_(\d)+d/gi, '') + ' cursor_' + ((d + 45) % 180) + 'd';
          resizer.sw_resizer.className =
            resizer.sw_resizer.className.replace(/ cursor_(\d)+d/gi, '') + ' cursor_' + ((d + 45) % 180) + 'd';
          resizer.nw_resizer.className =
            resizer.nw_resizer.className.replace(/ cursor_(\d)+d/gi, '') + ' cursor_' + ((d + 135) % 180) + 'd';
          resizer.se_resizer.className =
            resizer.se_resizer.className.replace(/ cursor_(\d)+d/gi, '') + ' cursor_' + ((d + 135) % 180) + 'd';
          resizer.n_resizer.className =
            resizer.n_resizer.className.replace(/ cursor_(\d)+d/gi, '') + ' cursor_' + ((d + 0) % 180) + 'd';
          resizer.s_resizer.className =
            resizer.s_resizer.className.replace(/ cursor_(\d)+d/gi, '') + ' cursor_' + ((d + 0) % 180) + 'd';
          resizer.w_resizer.className =
            resizer.w_resizer.className.replace(/ cursor_(\d)+d/gi, '') + ' cursor_' + ((d + 90) % 180) + 'd';
          resizer.e_resizer.className =
            resizer.e_resizer.className.replace(/ cursor_(\d)+d/gi, '') + ' cursor_' + ((d + 90) % 180) + 'd';
        }
      };
      if (!resizer.visibile) {
      ...
      resizer.style.left = parseInt(bBox.left - bBoxContainer.left) + 'px';
      resizer.style.top = parseInt(bBox.top - bBoxContainer.top) + 'px';
